latfit <- function(freq, ID="NULL", folder, system){
  library(ggplot2)
  ...
  if(!is.null(ID)){
    ...
    MyTest <- data.frame(newdata$Level.dB., newdata$T1.ms.)
  }
  ...
  if(!is.null(ID)){
    my_plot <- my_plot + geom_point(size=3) +  
      geom_point(data= MyTest,aes(newdata.Level.dB., newdata.T1.ms.), size=3, colour="red") 
  }
